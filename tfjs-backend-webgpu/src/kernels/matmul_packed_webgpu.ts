...
export function makeMatMulPackedWGSLSource(workPerThread: number[]): string {
...
export function makeMatMulVectorWGSLSource(): string {
...
export class MatMulPackedProgram implements WebGPUProgram {
  ...
  [[block]] struct Matrix {
    ...
    [[group(0), binding(3)]] var<uniform> uniforms : Uniforms;
      ...
      //let dimAOuter : u32 = ${
        this.transposeA === true ? `uniforms.aShape[2]` :
                                   `uniforms.aShape[1]`};
      //let dimInner : u32 = ${
        this.transposeA === true ? `uniforms.aShape[1]` :
                                   `uniforms.aShape[2]`};
      //let dimBOuter : u32 = ${
        this.transposeB === true ? `uniforms.bShape[1]` :
                                   `uniforms.bShape[2]`};
