...
export function getSharedArray1DCodeWgsl(shareMmemorySize: number) {
  return `
  var<workgroup> mm_Asub : array<vec4<f32>, ${shareMmemorySize}>;`;
}
...
function getPackedVec4MainCodeWgsl(
    varSnippet: string, workGroupSize: [number, number, number],
    tileInfo: TileInfo) {
  return `
  ...
  ${getWorkGroupSizeString(workGroupSize)}
...
function getVectorVec4MainCodeWgsl(workGroupSize: [number, number, number]):
    string {
  return `
...
export function makeMatMulPackedVec4SourceWgsl(
  ...
  const kMatMulVec4TwoDimensionalSharedArray =
      ...
      getPackedVec4MainCodeWgsl(varSnippet, workGroupSize, tileInfo);
...
export function makeMatMulVectorVec4SourceWgsl(
  ...
  const kMatMulVec4OneDimensionalSharedArray =
      getSharedArray1DCodeWgsl(workGroupSize[0] / 4) +
      getVectorVec4MainCodeWgsl(workGroupSize);
...
export class MatMulPackedVec4Program implements WebGPUProgram {
  ...
  getUserCodeWgsl(): string {
    ...
    const varSnippet = `let dimInner = uniforms.aShape[2];`;
    const userCode = `
      ${activationSnippet}  
      ...
      fn mm_write(row : u32, col : u32, value : vec4<f32>, globalId : vec3<u32>) {
