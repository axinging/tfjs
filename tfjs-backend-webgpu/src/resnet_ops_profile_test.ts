...
describeWebGPU('Ops resnetconvprofile', () => {
  ...
  function averageKernelTime(kernels: any, trials: any, reps: any) {
    let aggregatedKernelTime = 0;
    kernels.forEach((kernel: {name: string|number; kernelTimeMs: any;}) => {
      aggregatedKernelTime = aggregatedKernelTime + kernel.kernelTimeMs;
    });
    const frequencyMultipler = 83.3;

    return (aggregatedKernelTime * frequencyMultipler / (trials * reps))
        .toFixed(3);
  }
  ...
  function testConv2d(
    ...
    it(`conv2d xShape:${xShape} fShape:${fShape} stride:${stride} pad:${
       ...
       async () => {
         ...
         kernelInfo.kernels = kernelInfo.kernels.sort(
             (a: any, b: any) => b.kernelTimeMs - a.kernelTimeMs);
         // console.log(JSON.stringify(kernelInfo.kernels));
         console.log(
             `conv2d xShape:${xShape} fShape:${fShape} stride:${stride} pad:${
                 pad} format:${format} ` +
             averageKernelTime(kernelInfo.kernels, nTrail, nRep) + 'ms');
       });
  }
